...
p_prof = np.genfromtxt("../Photos/men.txt",delimiter=' ',dtype=None, encoding=None)
p_stud = np.genfromtxt("../Photos/leandro.txt",delimiter=' ',dtype=None, encoding=None)
...
for n in range(nPoints):
    ...
    if (n == 6):
        ...
        cv2.circle(frame, tuple(partC), 8, (0, 0, 255), thickness=-1, lineType=cv2.FILLED)
    else:   
        cv2.line(frame, tuple(partA), tuple(partB), (0, 255, 255), 3)
        cv2.circle(frame, tuple(partA), 8, (0, 0, 255), thickness=-1, lineType=cv2.FILLED)
        cv2.circle(frame, tuple(partB), 8, (0, 0, 255), thickness=-1, lineType=cv2.FILLED)
        cv2.circle(frame, tuple(partC), 8, (0, 0, 255), thickness=-1, lineType=cv2.FILLED)
